...
public class BossBehaviour : MonoBehaviour {
    ...
    {
        isThrowing = true;
        Throwable obj = Instantiate<Throwable>(weapons[Random.Range(0, weapons.Count)], sightPoint[Random.Range(0, sightPoint.Count)].position, Quaternion.identity);
    ...
    public List<Transform> sightPoint;
    bool PlayerOnSight()
    {
        foreach (Transform point in sightPoint)
        {
            RaycastHit2D hit;
            hit = Physics2D.Raycast(point.position, (new Vector3(transform.localScale.x, 0, 0)).normalized, enemyLineSight);
